...
const concatWithImagePath = (path) =>
  R.pipe(getPictureListMaisBien, R.map(R.concat(`${path}/`)))(path);
...
const loadImages = (rootPath) =>
  R.pipe(fs.readdir, R.andThen(bMap(readImage_(rootPath))))(rootPath);

// Rootpath -> dir creation
const createAnimalDirectory = (rootPath) =>
  R.pipe(
    R.map(
      R.pipe(
